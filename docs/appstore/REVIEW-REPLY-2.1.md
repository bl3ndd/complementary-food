...
> **✅ ЗАКРЫТО 04.09.2026.** Приложение одобрено, 1.0.0 в состоянии `READY_FOR_SALE`
> с билдом 5. Сработала связка: ответ по семи пунктам в Resolution Center + демо,
> переснятое на живом iPhone (первое было с симулятора — именно к этому и была
> претензия) + новый билд. Документ оставлен как образец: шаблон 2.1 присылают
> часто, и текст ответа переиспользуем.
