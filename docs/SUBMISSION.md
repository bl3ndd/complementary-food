...
## ✅ 1.0.0 В ПРОДАЖЕ (04.09.2026)

Проверено через ASC API: версия 1.0.0 в состоянии `READY_FOR_SALE`, прицеплен
**билд 5** (залит 02.09, `VALID`). Реджект «Guideline 2.1 — Information Needed»
закрыт связкой: ответ в Resolution Center по всем семи пунктам + пересняток демо
на живом iPhone (первое видео было снято на симуляторе, за что и завернули) +
свежий билд.

Что это разблокирует:
- **Переезд на свой домен** — менять Privacy Policy URL и Support URL в ASC теперь
  безопасно (см. план в памяти: `babyfooddiary.com`, этап 2).
- **Покупка Pro** — можно заводить non-consumable и переписывать Review Notes
  (§8а в `ASC-METADATA.md`); раньше это было опасно делать на версии в ревью.
- Следующий релиз — 1.1.0 с Pro и несколькими детьми.

⚠️ **Про `Entitlements.defaultProVersion`.** Сейчас там `1.1.0`, значит все, кто
поставил 1.0.0, получают Pro бесплатно навсегда — это и есть обещание ранним.
Но 1.0.0 теперь в открытой продаже, и круг «ранних» растёт каждый день до выхода
1.1.0. Это осознанная цена обещания, но чем дольше 1.1.0 не выходит, тем больше
людей получат Pro даром.
