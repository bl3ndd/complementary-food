...
> **Статус: закрыт** (2026-07-27) — все 22 задачи и критерии приёмки выполнены.
>
> Расхождения с изначальным планом (сделано иначе/шире):
> - Task 7: тексты «чем полезен» заведены не для 5–10 образцов, а **для всех 71 продукта**.
> - Task 13: лист правки заметки называется `EditLogSheet.swift` (в плане — `EditNoteSheet.swift`)
>   и правит весь лог (оценка/реакция/заметка), не только заметку.
> - Task 19: локализация ушла дальше EN — **14 языков** (см. `completed/20260628-localization.md`).
> - Task 6: в каталоге 71 продукт (вода удалена, псевдо-категория `allergen` разобрана
>   по настоящим группам).
...
[x] В `FeedingService.logFeeding(_:liking:reaction:date:)` убрать блок
    `if let reaction { s.state = isMaintenance ? .allergy : .paused }` — состояние от
    реакции больше НЕ меняется; лог пишется как раньше.
[x] В `ReactionType` (Enums.swift) добавить кейсы `constipation`, `diarrhea`; задать им
...
[x] Обновить инфо-текст в `LogFeedingSheet` (см. Task 11) — убрать «переведёт в паузу».
[x] Тесты: `logFeeding` с реакцией в `.introducing` НЕ переводит в `.paused`; в
...
[x] прогнать весь набор тестов — должен быть зелёным перед Task 2
...
[x] `IntroductionStatus`: добавить `var retryAt: Date? = nil` (CloudKit-safe дефолт).
[x] `FeedingService.stopIntroduction(_ food:)` — `.introducing`/`.introduced` → `.paused`,
    `retryAt = nil`.
[x] `FeedingService.scheduleRetry(_ food:, after months: Int = 2, now: Date = Date(),
    calendar: Calendar = .current)` — оставляет `.paused`, ставит
    `retryAt = calendar.date(byAdding: .month, value: months, to: now)`.
[x] `FeedingService.reintroduce(_:date:)` (расширяем сигнатуру в Task 3) — также чистит
    `retryAt = nil`.
[x] Оставить `markAllergy(_:)` как **ручную** пометку проблемы для уже введённого
    (гасит напоминания поддержки — `AllergenMaintenance` уже фильтрует `hasAllergy`).
[x] `NotificationManager`: новый `retryPrefix = "retry-"`. Метод
    ...
    и `retry-`.
[x] Тесты (`FeedingServiceTests`): `stopIntroduction` → `.paused`; `scheduleRetry` ставит
...
[x] прогнать весь набор тестов — должен быть зелёным перед Task 3
...
[x] `startIntroduction(_ food:, date: Date = Date())` — `introStartedAt = date`, intro-лог
    с `date` (сейчас всегда `Date()`).
[x] `reintroduce(_ food:, date: Date = Date())` — `introStartedAt = date`.
[x] В `logFeeding`: если `s.state == .introducing` и тип intro и `date < s.introStartedAt`
    → подтянуть `introStartedAt = date` (бэкдейт двигает окно).
[x] Чистый хелпер `static func windowStart(introStartedAt: Date?, introLogDates: [Date])
...
[x] Тесты: старт с прошлой датой → `observationDay` считает от неё; бэкдейт-кормление в
...
[x] прогнать весь набор тестов — должен быть зелёным перед Task 4
...
[x] `FeedingProfile`: удалить статические `who`/`aap`/`russia`, `visiblePresets(language:)`,
    `preset(id:)`, поля `isPreset`/`source`/`sourceURL`/`caveat`. Оставить `id`, `name`,
    параметры, `custom(from:)`, `customId`, `CustomLimits`.
[x] `Child.feedingProfile` — всегда `FeedingProfile.custom(from: self)`; `feedingProfileId`
    можно зафиксировать на `customId` (оставить поле ради CloudKit-совместимости).
[x] Удалить `MethodologyCard.swift`; вычистить импорты/использования (Onboarding — Task 17,
    Profile — Task 15).
[x] Найти тесты, ассертящие значения пресетов (grep по `who`/`aap`/`russia`/`visiblePresets`)
    — удалить/переписать на кастом-профиль.
[x] Напомнить юзеру `xcodegen generate` (удалён файл).
[x] Тесты: `FeedingProfile.custom(from:)` отдаёт корректные параметры из `Child`; нет
    ссылок на удалённые символы (компиляция = тест).
[x] прогнать весь набор тестов — должен быть зелёным перед Task 5
...
[x] `Child`: `customObservationDays` → `customObservationDaysRegular: Int = 3` +
    `customObservationDaysAllergen: Int = 5` (CloudKit-safe дефолты).
[x] `FeedingProfile`: `observationDays` → `observationDaysRegular` + `observationDaysAllergen`;
    `custom(from:)` мапит оба поля.
[x] Хелпер `FeedingProfile.observationDays(for food: Food) -> Int` = allergen-окно если
    `food.isAllergen`, иначе regular. Использовать в `FoodDetailView` (Task 11) и при
    подсчёте `isObservationComplete`.
[x] Тесты: для аллергена берётся allergen-окно, для обычного — regular; `custom(from:)`
    переносит оба значения из `Child`.
[x] прогнать весь набор тестов — должен быть зелёным перед Task 6
...
[x] `foods.json`: удалить запись `water`. Перекатегоризировать все `"category":"allergen"`
    ...
    `allergenGroup`.
[x] `FoodCategory`: удалить кейс `.allergen`; обновить все исчерпывающие `switch`
    (`title`, `Theme.categoryColor`, любые иконки `cat_allergen`).
[x] Тесты: декод `foods.json` — нет продукта с id `water`; нет продукта с категорией
    `allergen` (кейса больше нет); каждый `isAllergen==true` имеет `allergenGroup != nil`;
    счётчик продуктов обновлён (72 → актуальное).
[x] прогнать весь набор тестов — должен быть зелёным перед Task 7
...
[x] `Food`: добавить `let benefits: String?` и `let nutrients: [String]?` (опциональны →
    старый JSON декодится). `localizedBenefits` через `String(localized:)` если ключ-строка.
[x] `foods.json`: добавить поля для **5–10 продуктов** как образец (кабачок, брокколи,
    мясо, яйцо, рыба…). Остальные — placeholder (помечено, наполняется позже).
[x] Добавить RU-ключи + EN в `Localizable.xcstrings` для образцовых текстов (Task 18
    финализирует остальные).
[x] Тесты: декод записи с `benefits`/`nutrients` и без них (оба валидны); `nutrients`
    парсится в массив.
[x] прогнать весь набор тестов — должен быть зелёным перед Task 8
...
[x] Переписать `search(_ query:) -> [Food]`: нормализация (lowercase, диакритика, ё→е),
...
[x] Матч по группе: если запрос совпадает с `FoodCategory.title` (напр. «овощи») или
    `AllergenGroup.title` — добавить все продукты этой группы в результат (п.17).
[x] Тесты: «кабачёк»/«кабчок» (опечатка) находит «Кабачок»; вхождение в середине слова
    находит; «овощи» возвращает все `vegetable`; ранжирование (точное выше fuzzy); пустой
    запрос → все.
[x] прогнать весь набор тестов — должен быть зелёным перед Task 9
...
[x] Отступ поиска (п.2): уменьшить зазор под `searchBar` — поправить `listRowInsets`
    (сейчас top:8/bottom:4) и/или верхний инсет `List`; убрать лишний воздух до первой
    секции.
[x] Кнопка «Добавить свой продукт» (п.19): вынести `addCustomRow` из секции `.other`
    наверх — заметной строкой под `searchBar` (или `ToolbarItem` `+`). Убрать её из
    `.other`.
[x] Бейдж «аллерген» (п.14): заменить громкий оранжевый `StatusBadge("аллерген")` в
...
[x] Поиск-группа (п.17): UI уже отрендерит результат из обновлённого `catalog.search`
    (Task 8) — проверить, что секции категорий показывают найденную группу.
[x] Без юнит-тестов (leaf UI; логика поиска покрыта Task 8). Проверка self-launch:
...
[x] Центрирование/паддинг: убрать обрезание/смещение эмодзи-фолбэка (baseline) — рендерить
    эмодзи в контейнере фикс. размера с центровкой; единый размер для `food_<id>` и
    `cat_<category>`.
[x] Аудит: какие `food_<id>` ассеты отсутствуют и валятся в `cat_<category>`/эмодзи;
    список на догенерацию (placeholder-арт позже).
[x] Без юнит-тестов (рендер). Проверка self-launch: ряд каталога и герой деталки.
...
[x] Каунтер истории (п.3): удалить `Text("\(logs.count)")` в `historyCard`.
[x] Кнопки по состоянию (`actionButtons`):
...
[x] Реакция (п.4): «Была реакция» открывает `LogFeedingSheet`, но без авто-перехода (Task
    1) — просто фиксирует запись; обновить инфо-текст листа (убрать «переведёт в паузу»).
[x] Секция «Чем полезен» (п.12): показать `food.benefits`/`nutrients` если есть (чипами/
    текстом) под героем.
[x] Окно наблюдения: `observationDay`/`canComplete` считать через `windowStart` (Task 3) и
    `observationDays(for: food)` (Task 5).
[x] Без отдельных юнит-тестов (UI поверх покрытых сервисов). Проверка self-launch: все
...
[x] Заметку писать в **тот же** `FoodLog` кормления (передавать `note` в
...
[x] Reaction picker: показать новые теги (`constipation`/`diarrhea`, переименованные
    `gi`/`breathing`) — `ReactionType.allCases` уже отрендерит.
[x] Тесты: `logFeeding(..., note:)` сохраняет заметку в том же логе; отдельная запись
    заметки больше не создаётся.
[x] прогнать весь набор тестов — должен быть зелёным перед Task 13
...
[x] `EditNoteSheet(log: FoodLog)` — `TextField` с `log.note`, сохранение в `log.note` +
    `context.save()`.
[x] В `DayDetailView` и `historyRow` (FoodDetailView): тап по записи/заметке открывает
    `EditNoteSheet`. Показывать заметку в `DayDetailView` (сейчас не рендерится).
[x] Без юнит-тестов (UI + прямое присваивание `log.note`). Проверка self-launch: правка и
    сохранение заметки.
...
[x] Цвета (п.6): «есть записи» сменить с `Theme.accent` (коралл) на холодный
    (`Theme.sky`/`Theme.mint`), «реакция» оставить `.red`; обновить `legend`. Для плановых
    — третий стиль (пунктир/`Theme.lilac`).
[x] `FoodLog.planned: Bool = false`. Планирование: тап по будущему дню → лист «запланировать
    ввод» (создаёт `FoodLog(planned:true, date:future)`); `canGoNext`/навигация разрешает
    будущие месяцы.
[x] `CalendarService`: `activeDays()`/`days()`/`day()` учитывают плановые; `DaySummary`
    помечает плановость; будущие дни не считаются «реакцией».
[x] `DayDetailView`: плановые записи отдельным стилем; «отметить выполненным» снимает
    `planned`.
[x] Тесты: `CalendarService` группирует будущий плановый лог в нужный день; `hasActivity`/
    плановость различаются; граница дня/таймзона детерминированы (инъекция `calendar`).
[x] прогнать весь набор тестов — должен быть зелёным перед Task 15
...
[x] Отступ сверху (п.5): `.navigationBarTitleDisplayMode(.inline)` для «Сегодня» (Каталог/
    деталь уже inline) ИЛИ кастом-хедер; ужать верхний `.padding()`/spacing до первого блока.
[x] Блок ребёнка (п.24): переработать `heroCard` — компоновка имени/возраста/прогресса
    (макет на self-launch; держать `ProgressRingOnColor`, `Mascot.forProgress`).
[x] Раздел «Повторить ввод» (п.16): показать продукты с `retryAt`/`.paused` с датой
    «попробовать ~`retryAt`».
[x] Без юнит-тестов (leaf UI). Проверка self-launch: отступ, новый блок, раздел повторов.
...
[x] Имя (п.25): строка `LabeledContent("Имя") { TextField(...) }` или HStack «Имя» слева +
    значение/поле справа (сейчас просто инлайн `TextField`).
[x] Отступ (п.5): `.navigationBarTitleDisplayMode(.inline)` для «Профиль».
[x] План (п.11): вместо `MethodologyCard(expanded:)` + пикера пресетов — встроить
    `CustomPlanEditor(child: child)` (своя методика).
[x] Без юнит-тестов (leaf UI). Проверка self-launch: верстка имени, редактор плана.
...
[x] Замена +/- (п.9): значения (старт/окно/частота) выбирать тапом по бейджам/сегментам
    (ряд кликабельных капсул со значениями) вместо `roundButton("minus"/"plus")`.
[x] Второй степпер окна (п.10): отдельные контролы «окно обычного» и «окно аллергена»
    (`customObservationDaysRegular`/`customObservationDaysAllergen`, Task 5).
[x] Без юнит-тестов (UI поверх покрытой модели). Проверка self-launch: тап-выбор, два окна.
...
[x] Шаг плана (п.8): заменить выбор пресетов (`visiblePresets`/`MethodologyCard`, удалены
    Task 4) на встроенный `CustomPlanEditor(child: draftChild)`.
[x] Шаг «уже введено» (п.23): мульти-выбор продуктов из каталога → на финише создать
    `IntroductionStatus(state: .introduced)` для каждого. Вынести в
    `FeedingService.markIntroduced(_ foods:)` (тестируемо).
[x] `lastStep` и индикатор шагов обновить под новое число шагов.
[x] Тесты: `markIntroduced([...])` создаёт статусы `.introduced` без дублей.
[x] прогнать весь набор тестов — должен быть зелёным перед Task 19
...
[x] Интерполяции через формат-строки (не `\()`): `DashboardView` стр.101 (ранний баннер),
    стр.126 (если есть), стр.139 (`"%lld мес"`/`"\(ageInMonths) мес"`); проверить новые
    строки из Task 11/15.
[x] `Disclaimer.short`/`Disclaimer.welcome` обернуть в `String(localized:)`.
[x] Добавить EN для всех новых ключей (стоп/возобновить/повторить, теги реакции, «чем
    полезен», заголовки планирования, лейбл «Имя» и т.д.) + 5 ранее недостающих.
[x] Тесты: `Disclaimer.short`/`welcome` непустые/локализуются; новые модельные строки
    (`ReactionType.title` новых кейсов) непустые в обоих языках (locale-robust ассерты).
[x] прогнать весь набор тестов — должен быть зелёным перед Task 20
...
[x] Проверить на узких ширинах (320–414px): переполнения, длинные строки, sticky-CTA,
    отступы. Точечные фиксы (он уже адаптивный — viewport/`clamp()`/grid auto-fit/медиа
    720px).
[x] Добавить промежуточный брейкпоинт при необходимости (480px) для карточек/шагов.
[x] Без юнит-тестов (статичный сайт). Проверка: DevTools responsive RU+EN.

### Task 21: Проверка критериев приёмки

[x] Поиск находит по опечатке/вхождению; «овощи» показывает группу овощей (п.1, п.17).
[x] Отступ под поиском адекватный; кнопка «свой продукт» сверху (п.2, п.19).
[x] В истории нет каунтера (п.3).
[x] Тап «была реакция» = понятная запись без авто-перехода; есть «Остановить ввод», а в
    паузе — «Возобновить» и «Попробовать через 2 мес» с уведомлением (п.4, п.16).
[x] Отступ сверху на Главной/Календаре/Профиле уменьшен (п.5).
[x] Цвета «есть записи» и «реакция» в календаре различимы (п.6).
[x] EN-локализация без дыр; дисклеймер локализован (п.7).
[x] Настройка плана в онбординге; выбор уже введённых продуктов работает (п.8, п.23).
[x] В редакторе плана выбор тапом по бейджу; два окна наблюдения (п.9, п.10).
[x] Пресетов методик нет — только своя (п.11).
[x] У продуктов есть секция «чем полезен» (хотя бы образцы) (п.12).
[x] Иконки ровные (п.13).
[x] Каталог перестроен; псевдо-категория `allergen` убрана; воды нет (п.14, п.18).
[x] Теги реакции: ЖКТ → запор/диарея; «дыхание» прояснено; ЖКТ не уходит в аллергию (п.15).
[x] Заметку можно отредактировать (п.20).
[x] Календарь умеет планировать на будущее (п.21).
[x] Бэкдейт-кормление двигает окно/засчитывается (п.22).
[x] Блок ребёнка переработан (п.24); имя в профиле — лейбл+значение (п.25).
[x] Лендос ок на мобильных (п.26).
[x] Вся новая логика покрыта юнит-тестами (сервисы/модели ≥80%).
...
[x] `SPEC.md`: переписать §4.4 (реакция = запись + ручной стоп/возобновить/повтор; нет
...
[x] `CLAUDE.md`: обновить раздел архитектуры (удалён `MethodologyCard`; новые поля моделей;
    `retry-` уведомления; fuzzy-поиск).
[x] `docs/legal/methodology-sources.md` — пометить, что пресеты убраны из UI (источники
    оставить для истории/возможного возврата).
[x] Напомнить юзеру `xcodegen generate` (удалён `MethodologyCard.swift`, добавлен
    `EditNoteSheet.swift`).
