...
struct FeedingProfile: Identifiable, Codable, Hashable {
    ...
    static let who = FeedingProfile(
        id: "who",
        name: String(localized: "ВОЗ / ESPGHAN (Европа)"),
        startAgeMonths: 6,
        observationDays: 3,
        allergenFrequencyPerWeek: 2,
        allergenGroups: [.egg, .peanut, .dairy, .gluten, .fish, .soy, .treenut, .sesame],
        isPreset: true,
        source: String(localized: "ВОЗ 2023 + ESPGHAN 2017 (JPGN)"),
        sourceURL: "https://www.who.int/publications/i/item/9789240081864",
        caveat: String(localized: "Старт по ВОЗ — 6 мес (ESPGHAN допускает с 4, окно 4–6). «3 дня наблюдения» — общий консенсус (вводить по одному продукту и следить за реакцией), а не прямая цифра ВОЗ/ESPGHAN. Частота поддержки аллергена — клинический консенсус из исследований LEAP/EAT, не буква гайдлайна.")
    )

    static let aap = FeedingProfile(
        id: "aap",
        name: String(localized: "AAP / NIAID (США)"),
        startAgeMonths: 6,
        observationDays: 3,
        allergenFrequencyPerWeek: 3,
        allergenGroups: [.peanut, .egg, .dairy, .gluten, .fish, .shellfish, .soy, .treenut, .sesame],
        isPreset: true,
        source: String(localized: "AAP (HealthyChildren) + NIAID 2017"),
        sourceURL: "https://www.healthychildren.org/English/ages-stages/baby/feeding-nutrition/Pages/Starting-Solid-Foods.aspx",
        caveat: String(localized: "Старт ~6 мес («around 6 months»). «3 дня» — низ официального диапазона AAP «3–5 дней». Список аллергенов = FDA «Big 9» (вкл. кунжут, FASTER Act). Частота 3×/нед — это пинат-протокол LEAP/NIAID, распространённый на другие аллергены как консенсус, а не отдельная цифра гайдлайна.")
    )

    static let russia = FeedingProfile(
        id: "russia",
        name: String(localized: "Союз педиатров РФ"),
        startAgeMonths: 5,
        observationDays: 7,
        allergenFrequencyPerWeek: 1,
        allergenGroups: [.egg, .dairy, .gluten, .fish],
        isPreset: true,
        source: String(localized: "Нац. программа оптимизации вскармливания детей 1-го года жизни в РФ"),
        sourceURL: "https://zdravdeti.org/wp-content/uploads/2022/11/prog-vskarm.pdf",
        caveat: String(localized: "Старт 5 мес — дословный оптимум программы для ГВ. Окно 7 дней — верх диапазона «вводить за 5–7 дней». Важно: регулярной «поддержки аллергена для толерантности» в программе РФ нет (она прямо считает раннее введение аллергенов требующим дальнейших исследований) — параметр оставлен как осторожный ориентир по западному консенсусу.")
    )
    ...
    static func custom(from child: Child) -> FeedingProfile {
        FeedingProfile(
            ...
            isPreset: false,
            source: String(localized: "Свой план — настроен вами"),
            sourceURL: "https://pudding-for-children.vercel.app/#method",
            caveat: String(localized: "Это ваши собственные настройки, а не клиническая рекомендация. Сроки введения продуктов и аллергенов согласуйте с педиатром.")
        )
    }
}
