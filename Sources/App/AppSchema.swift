...
/// `FoodLog.childId` и `IntroductionStatus.childId` (03.09.2026) добавлены сюда же,
/// БЕЗ новой версии — по тому же правилу: это поля у существующих типов. Записи,
/// сделанные до появления нескольких детей, получают `nil` и подхватываются разовым
/// бэкфиллом `PlanMigration.ChildOwnership`.
